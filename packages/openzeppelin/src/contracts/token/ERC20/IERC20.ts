...
export interface IERC20 extends NgContract<IERC20Events> {
...
export function isIERC20(contract: Contract): contract is IERC20 {
  return IERC20Abi.filter((def) => def.type === "function").every((def) => def.name && def.name in contract.functions);
}
