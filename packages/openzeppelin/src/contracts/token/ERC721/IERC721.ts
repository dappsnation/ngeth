...
export interface IERC721 extends NgContract<IERC721Events> {
...
export function isIERC721(contract: Contract): contract is IERC721 {
  return IERC721Abi.filter((def) => def.type === "function").every((def) => def.name && def.name in contract.functions);
}
