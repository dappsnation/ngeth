  ...
  if (paramType.baseType === 'array') {
    if (!Array.isArray(value)) {
      return Promise.reject(
        `invalid value for array: ${JSON.stringify(value)}`
      );
    }
    return await Promise.all(
...
export async function populateTransaction(
  ...
  if (!signerOrProvider)
    throw new Error('Provider is required to populateTransaction');
  ...
  if (ro.value) {
    ...
    if (!roValue.isZero() && !fragment.payable) {
      throw new Error(
        `non-payable method cannot override value: ${JSON.stringify(
          overrides.value
        )}`
      );
    }
