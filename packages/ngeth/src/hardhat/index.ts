...
task('ngeth:test', 'run test with jest', async (taskArguments: any, hre: any) => {
...
task('ngeth:build', '', async (taskArguments: any, hre: HardhatRuntimeEnvironment) => {
  return hre.run('compile', taskArguments);
});

task('ngeth:serve', '',  async (taskArguments: any, hre: HardhatRuntimeEnvironment) => {
