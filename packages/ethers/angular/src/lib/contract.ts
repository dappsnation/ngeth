...
// TODO: find a way to keep type for the arguments
...
export function ngContract<
  Contract extends EthersContract<Events, EventKeys, FilterKeys>,
  Events extends ContractEvents<EventKeys, FilterKeys> = GetEvents<Contract>,
  EventKeys extends Extract<keyof Events['events'], string> = Extract<keyof Events['events'], string>,
  FilterKeys extends Extract<keyof Events['filters'], string> = Extract<keyof Events['filters'], string>,
>(contract: Constructor<Contract>): Constructor<NgContract<Contract, Events, EventKeys, FilterKeys>> {
  class NgEthersContract extends (contract as Constructor<EthersContract<Events, EventKeys, FilterKeys>>) {
    private ngZone: NgZone;
    private _events: Record<string, Observable<any>> = {};
  
    constructor(...args: ConstructorParameters<typeof contract>) {
      super(...args);
      const ngZone = args.find(arg => arg instanceof NgZone);
  ...
  return NgEthersContract as any;
}
